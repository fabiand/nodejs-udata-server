...
udata.API = function() {
    ...
    Connection.prototype.request = function connection_request(path, options, callback) {
        ...
        options.headers["x-udata-timestamp"] = timestamp()
        this.sign_request(options, ["x-udata-method", "x-udata-url", "x-udata-timestamp"])
console.log("Requesting: " + options.toString())
    ...
    PubSub.prototype.subscribe = function pubsub_subscribe(channel, callback) {
        ...
        var url = this._channel_url(channel);
        url += "?timestamp=" + timestamp()
        url += + "&signature=" + this.connection.signer.sign(url);
